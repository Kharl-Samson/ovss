...
export default function Top_Nav_Part(){
  ...
  useEffect(() => {
    ...
    const AcceptScheduleForm=(e)=>{
        e.preventDefault();
        //Sending the data request to call it on backend
        const sendData = {
          key : document.getElementById("notif_id").value,
          emailTo : document.getElementById("notif_email").value,
  ...
  const RejectScheduleForm=(e)=>{
    ...
    //Sending the data request to call it on backend
    const sendData = {
        key : document.getElementById("notif_id1").value,
        emailTo : document.getElementById("notif_email1").value,
  ...
  const Each_notif = appointments.map((res) => {
      ...
      return (
        ...
        formAccept = {AcceptScheduleForm}
        formReject = {RejectScheduleForm}
        />
      ); 
    }
  });
