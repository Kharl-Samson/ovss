...
export default function Patient_Profile_Page(){
  let navigate = useNavigate();

  //If local storage email has no value it can't acces admin pages
  const [auth, setAuth] = useState("");
  useEffect(() => {
  var auth = localStorage.getItem("patient_account_status");
  if (auth === "Pending") {
    navigate(`/Complete_Account`);
  }
  setAuth(auth);
  }, []);
  ...
  document.title = "OVSS | My Account";
  ...
  setTimeout(function () {
    ...
    var child_id = localStorage.getItem("patient_login_id");
